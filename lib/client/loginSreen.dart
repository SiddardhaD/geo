...
class _LoginState extends State<Login> {
  ...
  settingAttendance() async {
    ...
    if (_isLogIn) {
      ...
      await empLocationBox.put(
          'LoginEntry${widget.index + 1}',
          Employeedata(
              longsrc: _locationData.longitude,
              latisrc: _locationData.latitude,
              loginTime: tsdate.toString(),
              status: 1));
      ...
      DateTime tsdate =
          DateTime.fromMillisecondsSinceEpoch(_locationData.time!.toInt());
      await empLocationBox.put(
        'LogoutEntry${widget.index + 1}',
        Employeedata(
            longdes: _locationData.longitude,
            latides: _locationData.latitude,
            logoutTime: tsdate.toString(),
            status: 0),
      );
  ...
  onLocationChnage() {
    location.onLocationChanged.listen((LocationData loc) {
      debugPrint(
          "Current location ${widget.index + 1}: ${loc.latitude}, ${loc.longitude}");

      try {
        setState(() {
          var lat = loc.latitude!;
          var lon = loc.longitude!;
          empLocationBox.put("Live${widget.index + 1}",
              Employeedata(liveLati: loc.latitude, liveLong: loc.longitude));
        });
      } catch (e) {
  ...
  _addPolyLine() {
    ...
    Polyline polyline = Polyline(polylineId: id, color: Colors.red, points: [
      LatLng(17.433028, 78.3728344),
      LatLng(17.427043213495722, 78.37636184875586)
    ]);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Employee ${widget.index + 1}"),
      ),
      body: Container(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Align(
            alignment: Alignment.center,
            child: intializing != true
                ? _isLogIn == false
                    ? TextButton(
                        child: Text("LogIn"),
