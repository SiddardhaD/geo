...
class _EachEmployeeState extends State<EachEmployee> {
  ...
  Future<void> enablingHiveData() async {
    ...
    if (_isLogIn) {
      await getLocaion(
          empLocationBox.get('LoginEntry${widget.index + 1}')!.latisrc!,
          empLocationBox.get('LoginEntry${widget.index + 1}')!.longsrc!);
    }
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Details ${widget.index + 1}"),
      ),
      body: Container(
              ...
              onPressed: () {
                _isLogIn
                    ? Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => MonitorEmployee(
                                  index: widget.index,
                                  latitudeLive: empLocationBox
                                      .get("Live${widget.index + 1}")!
                                      .liveLati,
                                  longitudeLive: empLocationBox
                                      .get("Live${widget.index + 1}")!
                                      .liveLong,
                                  isLive: _isLogIn,
                                  latitudesrc: empLocationBox
                                      .get("LoginEntry${widget.index + 1}")!
                                      .latisrc,
                                  longitudesrc: empLocationBox
                                      .get("LoginEntry${widget.index + 1}")!
                                      .longsrc,
                            ...
                            MaterialPageRoute(
                                builder: (context) => MonitorEmployee(
                                    ...
                                    longitudesrc: empLocationBox
                                        .get("LoginEntry${widget.index + 1}")!
                                        .longsrc,
                                    latitudedes: empLocationBox
                                        .get("LogoutEntry${widget.index + 1}")!
                                        .latides,
                                    longitudedes: empLocationBox
                                        .get("LogoutEntry${widget.index + 1}")!
                                        .longdes)),
                          )
              ...
              ? hiveintializing == false
                  ? empLocationBox
                      ...
                      ? Padding(
                          ...
                          child: Text(
                              "Login Time  = ${empLocationBox.get('LoginEntry${widget.index + 1}')!.loginTime} "),
                        )
                      : Container()
                  : Container()
              : Text("Duty Not Started/Ended"),
          _isLogIn == true
