...
class MonitorEmployee extends StatefulWidget {
  ...
  const MonitorEmployee({super.key,required this.index, this.latitudeLive, this.longitudeLive,required this.isLive,this.latitudesrc,this.longitudesrc,this.latitudedes,this.longitudedes});
...
class _MonitorEmployeeState extends State<MonitorEmployee> {
  ...
  _liveAddMarker(LatLng position, String id, BitmapDescriptor descriptor){
    MarkerId markerId = MarkerId(id);
    Marker marker =
        Marker(markerId: markerId, icon: BitmapDescriptor.defaultMarkerWithHue(120), position: position,);
    markers[markerId] = marker;
  }
  @override
  void initState() {
    ...
    _addMarker(LatLng(widget.latitudedes!,widget.longitudedes!), "destination",
    ...
    getLogoutLocaion(widget.latitudedes!,widget.longitudedes!);
      ...
      _liveAddMarker(LatLng(widget.latitudeLive!,widget.longitudeLive!), "origin",
        BitmapDescriptor.defaultMarkerWithHue(90));
      // getLoginLocaion(widget.latitudesrc!,widget.longitudesrc!);
   ...
   _addPolyLine() {
    ...
    Polyline polyline = Polyline(
        ...
        width: 5,
        color: Colors.red, 
        points: [LatLng(widget.latitudesrc!,widget.longitudesrc!),LatLng(widget.latitudedes!,widget.longitudedes!)]);
          ...
          SizedBox(
            ...
            child: GoogleMap(
                initialCameraPosition: CameraPosition(
                    target: LatLng(17.433028, 78.3728344 ), zoom: 15),
                onCameraMove: (position) => CameraPosition(
                target: LatLng(17.427043213495722,78.37636184875586), zoom: 15),
            ...
            !widget.isLive?isFetched1==false?Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text("Login Location : $addressLine1"),
            ):Container():Container(),
            !widget.isLive?isFetched2==false?Padding(
              padding: const EdgeInsets.all(8.0),
              child: Text("Logout Location : $addressLine2"),
            ):Container():isFetched2==false?Padding(
              padding: const EdgeInsets.all(8.0),
